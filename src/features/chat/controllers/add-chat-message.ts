...
export class Add {
  ...
  public async message(req: Request, res: Response): Promise<void> {
    ...
    if(selectedImage.length) {
      ...
      fileUrl = `https://res.cloudinary.com/patribbb/image/upload/v${result.version}/${result.public_id}`
    }
    ...
    //NOTE: add sender to chat list in cache
    await messageCache.addChatListToCache(`${req.currentUser!.userId}`, `${receiverId}`, `${conversationObjectId}`);
    await messageCache.addChatListToCache(`${receiverId}`, `${req.currentUser!.userId}`, `${conversationObjectId}`);
    await messageCache.addChatMessageToCache(`${conversationObjectId}`, messageData)

    //NOTE: add message to chat queue
  ...
  private async messageNotification({currentUser, message, receiverName, receiverId}: IMessageNotification): Promise<void> {
    ...
    if(cachedUser.notifications.messages) {
      ...
      emailQueue.addEmailJob('directMessageEmail', {receiverEmail: currentUser.email, template, subject: `Ypu've received messages from ${currentUser.username}`})
    }
  }
}
